...
# Deforum settings save helpers
def _webui_root() -> Path:
    # .../extensions/<this_ext>/scripts/<file>.py -> parents[3] is webui root
    try:
        return Path(__file__).resolve().parents[3]
    except Exception:
        return Path.cwd()

def find_deforum_settings_dir() -> Optional[Path]:
    """Best-effort discovery of Deforum extension settings directory."""
    root = _webui_root()
    exts = root / "extensions"
    candidates: List[Path] = []
    if exts.exists():
        for d in exts.iterdir():
            if d.is_dir() and "deforum" in d.name.lower():
                for rel in [
                    Path("scripts") / "deforum" / "settings",
                    Path("scripts") / "deforum" / "settings_files",
                    Path("deforum") / "settings",
                    Path("settings"),
                ]:
                    p = d / rel
                    if p.exists() and p.is_dir():
                        candidates.append(p)
                for p in d.rglob("settings"):
                    if p.is_dir() and "deforum" in str(p).lower():
                        candidates.append(p)
    seen=set()
    uniq=[]
    for p in candidates:
...
def _safe_filename(name: str) -> str:
    name = (name or "").strip()
    if not name:
        name = "deforum_music_settings"
    name = re.sub(r'[\\/:*?"<>|]+', "_", name)
    return name

def save_settings_json_to_deforum(settings_json: str, filename_base: str = "") -> str:
    """Save generated settings to Deforum settings folder (if found)."""
    try:
        data = json.loads(settings_json) if settings_json else None
    except Exception as e:
        return f"❌ Settings JSON is not valid JSON: {e}"

    if not isinstance(data, dict):
        return "❌ Settings JSON must be an object/dict."

    settings_dir = find_deforum_settings_dir()
    if settings_dir is None:
        # Fallback: save under WebUI outputs so you can manually load/copy later
        settings_dir = _webui_root() / "outputs" / "deforum_music_settings"
        try:
            settings_dir.mkdir(parents=True, exist_ok=True)
        except Exception:
            return "❌ Could not locate Deforum settings folder and also could not create outputs/deforum_music_settings."

    ts = time.strftime("%Y%m%d_%H%M%S")
    base = _safe_filename(filename_base)
    out_path = settings_dir / f"{base}_{ts}.json"
    ...
    class DeforumMusicScript(scripts.Script):
        ...
        def ui(self, is_img2img):
            ...
            with gr.Group():
                ...
                with gr.Row():
                    settings_filename = gr.Textbox(
                        label="Deforum Settings Filename (base)",
                        value="deforum_music_settings",
                        lines=1
                    )
                    save_to_deforum_btn = gr.Button("Save JSON to Deforum Settings Folder")
                ...
                save_to_deforum_btn.click(
                    fn=save_settings_json_to_deforum,
                    inputs=[output_json, settings_filename],
                    outputs=[save_status]
                )
            
            return [audio_file, base_prompt, style_prompt, output_json, settings_filename, save_status]
        def run(self, p, audio_file, base_prompt, style_prompt, output_json, settings_filename, save_status):
